...
const SpanLeft = ({ hidden, children, onClick, title, style }: SpanProps) => {
  if (hidden) {
    return (
      <S.HiddenText style={{ ...style, float: 'left', marginLeft: '5px' }}>
        {children}
      </S.HiddenText>
    );
  }
  return (
    <span
      onClick={onClick}
      style={{ ...style, float: 'left', marginLeft: '5px', cursor: 'pointer' }}
      title={title}
    >
      {children}
    </span>
...
const SpanRight = ({ hidden, children, onClick, title, style }: SpanProps) => {
  if (hidden) {
    return (
      <S.HiddenText style={{ ...style, float: 'right', marginRight: '5px' }}>
        {children}
      </S.HiddenText>
    );
  }
...
export const Tray = (props: {
  ...
  const hasStart =
    starts.filter((val) => val.getOptions().title === 'Start').length !== 0;
            ...
            <button
              onClick={() => {
                ...
                if (value.length > 0 && !app.getTrees()[value]) {
                  ...
                  const activeTab = props.context.tabs.find(
                    (tab) => tab.id === props.context.activeTabId
                  );
                  if (activeTab) {
                    activeTab.activeTree = value;
                  }
          ...
          <S.CustomUI>
            ...
            <button
              onClick={() => {
                ...
                if (activeTab) {
                  activeTab.activeTree = undefined;
                }
  ...
  return (
    <>
      ...
      >
        ...
        <TabList>
          <Tab>Nodes</Tab>
          <Tab>{`Trees(${Object.keys(props.app.getTrees()).length + 1})`}</Tab>
        </TabList>
        <div
          style={{
            border: '1px solid',
        ...
        >
          <TabPanel>
            {
              <TrayItemWidget
                ...
                model={{
                  id: 'start',
                  extra: {
                    title: hasStart ? `Start-${size}` : 'Start',
                    editableTitle: hasStart,
                  },
                }}
                name={'start'}
          ...
          <TabPanel>
            <div style={{ color: '#ffffff' }}>
              {Object.entries(trees).map((entry, idx, arr) => {
                ...
                const selected =
                  props.app.getDiagramEngine().getModel() === value;
                return (
                  <div key={idx} style={{ margin: '0 10px' }}>
                    <S.Tree
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        if (!selected) {
                          props.context.tabs.forEach((tab) => {
                            if (tab.id === props.context.activeTabId) {
                              tab.activeTree = key;
                            }
                          });
          ...
          <div
            style={{
              position: 'absolute',
              bottom: '0',
              width: `clamp(134px, 20vw, ${ref.current?.offsetWidth || 200}px)`,
            }}
          >
